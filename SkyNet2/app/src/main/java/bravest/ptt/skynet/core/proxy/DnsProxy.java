...
import bravest.ptt.skynet.core.dns.Question;
import bravest.ptt.skynet.core.dns.Resource;
import bravest.ptt.skynet.core.dns.ResourcePointer;
...
public class DnsProxy implements Runnable {
        ...
        try {
            ...
            while (mClient != null && !mClient.isClosed()) {
                ...
                try {
                    ...
                    Log.e(TAG, "Parse dns error: " + ex);
                }
            }
        } catch (Exception e) {
            e.printStackTrace(System.err);
            Log.e(TAG, "DnsProxy Thread catch an exception " + e);
        } finally {
            Log.i(TAG, "DnsProxy Thread Exited.");
            this.stop();
        }
    }
        ...
        if (dnsPacket.Header.ResourceCount > 0) {
            ...
            if (question.Type == 1) {
                ...
                if (ProxyConfig.Instance.needProxy(question.Domain, realIP)) {
                    ...
                    Log.i(TAG, "FakeDns: " + question.Domain
    ...
    private void OnDnsResponseReceived(IPHeader ipHeader, UDPHeader udpHeader, DnsPacket dnsPacket) {
        ...
        if (state != null) {
            Log.i(TAG, "Received DNS result form Remote DNS Server");
            if (dnsPacket.Header.QuestionCount > 0 && dnsPacket.Header.ResourceCount > 0) {
                Log.i(TAG, "Real IP: " + dnsPacket.Questions[0].Domain
                        + " ==> " +  CommonMethods.ipIntToString(getFirstIP(dnsPacket)));
            }
            //DNS污染
    ...
    private boolean interceptDns(IPHeader ipHeader, UDPHeader udpHeader, DnsPacket dnsPacket) {
        ...
        Log.i(TAG, "DNS query " + question.Domain);
        ...
        if (question.Type == 1) {
            if (ProxyConfig.Instance.needProxy(question.Domain, getIPFromCache(question.Domain))) {
                ...
                Log.d(TAG, "interceptDns FakeDns: " + question.Domain
                        + " => " + CommonMethods.ipIntToString(fakeIP));
        ...
        if (!interceptDns(ipHeader, udpHeader, dnsPacket)) {
            ...
            try {
                if (VpnServiceHelper.protect(mClient)) {
                    ...
                    mClient.send(packet);
                    Log.i(TAG, "Send an DNS Request Package to Remote DNS Server "
                            +  CommonMethods.ipIntToString(state.mRemoteIP));
                } else {
                    Log.e(TAG, "VpnService protect udp socket failed.");
                }
            } catch (IOException e) {
                    e.printStackTrace(System.err);
                Log.e(TAG, "Send Dns Request Package catch an exception " +  e);
            }
        }
    }
