...
public class IPHeader {
     ...
     * 偏移单位：字节
     */
    //0: 版本号（4bits） + Internet头部长度IHL（4bits）
    static final byte offset_ver_ihl = 0;

    //1：TOS服务类型（8bits），前6位为DS字段（区分服务字段），后2位是显示拥塞通知（ECN）字段
    static final byte offset_tos = 1;
    //2：总长度字段，通过此字段和IHL字段，我们知道数据报的数据部分从哪里开始，由于这是一个16bits的字段，所以IP数据报的最大长度为2^16 即 65535字节
