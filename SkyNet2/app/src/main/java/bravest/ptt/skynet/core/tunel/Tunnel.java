...
import bravest.ptt.skynet.core.http.HttpResponse;
    ...
    public void onConnectable() {
        try {
            if (mInnerChannel.finishConnect()) {
                ...
                Log.e(TAG, "Connect to " + mServerEP + " failed.");
                this.dispose();
            }
        } catch (Exception e) {
            e.printStackTrace(System.err);
            Log.e(TAG, String.format("Connect to %s failed: %s", mServerEP, e));
            this.dispose();
        }
    }
...
//							}
    ...
    protected void sendToBrother(SelectionKey key, ByteBuffer buffer) throws Exception {
        if (isTunnelEstablished() && buffer.hasRemaining()) { //将读到的数据，转发给兄弟
            ...
            if (!mBrotherTunnel.write(buffer, true)) {
                ...
                Log.w(TAG, String.format("%s can not read more.\n", mServerEP));
            }
        }
    }
    ...
    public void onWritable(SelectionKey key) {
        try {
            ...
            ex.printStackTrace(System.err);
            Log.e(TAG, String.format("onWritable catch an exception: %s", ex));
            this.dispose();
        }
    }
    ...
    void disposeInternal(boolean disposeBrother) {
        if (!mDisposed) {
            try {
                ...
                ex.printStackTrace(System.err);
                Log.e(TAG, String.format("InnerChannel close catch an exception: %s", ex));
            }
